#! /usr/bin/env python
"""
Author: Muhammad Ali Haider Dar (ID: 5046263)

Description: In general, this script gets the user request, communicates with the my_srv server to get random index values, and lets the robot execute one of the pre-defined behaviors accordingly in the Gazebo environment, along with SLAM, path planning and collision avoidance.
"""
...
def main():
    """The architecture should be able to get the user request, and let the robot execute one of the predefined behaviors (depending on the user's input)."""

    """Here we initialize the ROS node"""
    rospy.init_node('final_ui')

    global target_reached_status, wall_follower_client

    """ Initializing client for random target index service """
    random_index_service = rospy.ServiceProxy('/final', Final)
...
(4) Stop in the last position.""")
	...
	if (x == 1):
	    ...
	    """ Directly ask the user for the next target position (checking that the position is one of the possible six) and reach it """
